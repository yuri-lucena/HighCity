...
--[ WEBHOOK ]----------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local logEntrada = "https://discordapp.com/api/webhooks/760881205767307304/6Vd-jNs5sppnbsV8wgU2PXloOM1YCrTqqdQVVva1NASFltc8F8BvVWkDVuyGSW5NU0te"
local logSaida = "https://discordapp.com/api/webhooks/760881205767307304/6Vd-jNs5sppnbsV8wgU2PXloOM1YCrTqqdQVVva1NASFltc8F8BvVWkDVuyGSW5NU0te"
			...
			PerformHttpRequest(logSaida, function(err, text, headers) end, 'POST', json.encode({
				embeds = {
					{ 
						...
						thumbnail = {
							url = "https://media.discordapp.net/attachments/393527746514649088/760899098223575080/rich.png"
						}, 
						fields = {
							{ 
								name = "**ID:**", 
						...
						footer = { 
							...
							icon_url = "https://media.discordapp.net/attachments/393527746514649088/760899098223575080/rich.png" 
						},
						color = 15914080 
					}
				}
...
AddEventHandler("queue:playerConnecting",function(source,ids,name,setKickReason,deferrals)
	...
	if ids ~= nil and #ids > 0 then
		deferrals.update("[HighCity] Carregando identidades.")
		local user_id = vRP.getUserIdByIdentifiers(ids)
		if user_id then
			deferrals.update("[HighCity] Carregando banimentos.")
			if not vRP.isBanned(user_id) then
				deferrals.update("[HighCity] Carregando whitelist.")
				if vRP.isWhitelisted(user_id) then
					if vRP.rusers[user_id] == nil then
						deferrals.update("[HighCity] Carregando banco de dados.")
					...
					deferrals.done("Solicite sua whitelist em nosso Discord! [ discord.gg/nYV6UaB ][ ID: "..user_id.." ]")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("Você foi banido! [ Mais informações em: discord.gg/nYV6UaB ] ")
				--print(user_id)
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
			deferrals.done("[HighCity] Ocorreu um problema de identificação.")
			TriggerEvent("queue:playerConnectingRemoveQueues",ids)
		end
	else
		deferrals.done("[HighCity] Ocorreu um problema de identidade.")
		TriggerEvent("queue:playerConnectingRemoveQueues",ids)
	end
end)
