...
--[ WEBHOOK ]----------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local discordDMLA = "https://discordapp.com/api/webhooks/761363869854597170/t582S1P1xphEIyC2j9C2_T04mpFlYT1ht-HUf4-p70X7BKRLFaiJbcYraSfK6RRBj4DA"
local discordDPLAe = "https://discordapp.com/api/webhooks/761363190222815242/eRi2glq6bvqKsm8wUDQkyCXy9WgP6UW_k8kJ1W_Db1pQv8uR4NMT3tOoiqhOBqippGzY"
local discordDPLAa = "https://discordapp.com/api/webhooks/761363190222815242/eRi2glq6bvqKsm8wUDQkyCXy9WgP6UW_k8kJ1W_Db1pQv8uR4NMT3tOoiqhOBqippGzY"
-----------------------------------------------------------------------------------------------------------------------------------------
--[ CHEST ]------------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local chest = {
	...
	["vanilla"] = { 5000, ['permissao'] = "vanilla.permissao"},
					...
					if new_weight <= parseInt(chest[tostring(chestName)][1]) then
						if vRP.tryGetInventoryItem(parseInt(user_id),itemName,parseInt(amount)) then
							...
							if chestName == "aDPLA" then
								PerformHttpRequest(discordDPLAa, function(err, text, headers) end, 'POST', json.encode({embeds = {{ title = "REGISTRO DE ARSENAL:⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀", thumbnail = {url = "https://media.discordapp.net/attachments/393527746514649088/760899098223575080/rich.png"}, fields = {{ name = "**Identificação do Colaborador:**", value = "` "..identity.name.." "..identity.firstname.." ["..user_id.."] `" },{ name = "Adicionou ao Arsenal:", value = "` [ Item: "..vRP.itemNameList(itemName).." ][ Quantidade: "..vRP.format(parseInt(amount)).." ]` \n " }}, footer = { text = os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S"), icon_url = "https://media.discordapp.net/attachments/393527746514649088/760899098223575080/rich.png" },color = 15906321 }}}), { ['Content-Type'] = 'application/json' })
							elseif chestName == "eDPLA" then
								PerformHttpRequest(discordDPLAe, function(err, text, headers) end, 'POST', json.encode({embeds = {{ title = "REGISTRO DE EVIDÊNCIAS:⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀", thumbnail = {url = "https://media.discordapp.net/attachments/393527746514649088/760899098223575080/rich.png"}, fields = {{ name = "**Identificação do Colaborador:**", value = "` "..identity.name.." "..identity.firstname.." ["..user_id.."] `" },{ name = "Adicionou ao Armário de Evidências:", value = "` [ Item: "..vRP.itemNameList(itemName).." ][ Quantidade: "..vRP.format(parseInt(amount)).." ]` \n " }}, footer = { text = os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S"), icon_url = "https://media.discordapp.net/attachments/393527746514649088/760899098223575080/rich.png" },color = 15906321 }}}), { ['Content-Type'] = 'application/json' })
							elseif chestName == "sDMLA" then
								PerformHttpRequest(discordDPLAe, function(err, text, headers) end, 'POST', json.encode({embeds = {{ title = "REGISTRO DE SUPRIMENTOS:⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀", thumbnail = {url = "https://media.discordapp.net/attachments/393527746514649088/760899098223575080/rich.png"}, fields = {{ name = "**Identificação do Colaborador:**", value = "` "..identity.name.." "..identity.firstname.." ["..user_id.."] `" },{ name = "Adicionou ao Armário de Suprimentos:", value = "` [ Item: "..vRP.itemNameList(itemName).." ][ Quantidade: "..vRP.format(parseInt(amount)).." ]` \n " }}, footer = { text = os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S"), icon_url = "https://media.discordapp.net/attachments/393527746514649088/760899098223575080/rich.png" },color = 15906321 }}}), { ['Content-Type'] = 'application/json' })		
							end
